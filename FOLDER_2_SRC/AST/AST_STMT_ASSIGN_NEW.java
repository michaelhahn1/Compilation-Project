...
public class AST_STMT_ASSIGN_NEW extends AST_STMT
{
	...
	public AST_STMT_ASSIGN_NEW(AST_VAR v, AST_NEW_EXP ne, int lineNumber) {
	...
	public TEMP IRme(){
		if (v.isVarField()){
			...
			IR.getInstance().Add_IRcommand(new IRcommand_Store_Register(instance, ne.IRme(), offset, lineNumber));
		}
		else{
			if(v.isSUBSCRIPT()){
				...
				IR.getInstance().Add_IRcommand(new IRcommand_Store_Register(address,ne.IRme(),0,lineNumber));
				...
				IR.getInstance().Add_IRcommand(new IRcommand_Set_Var(var, ne.IRme(), lineNumber));
			} else {
	...
	{System.out.print("Reached semantme of ASSIGN NEW\n");
		...
		if(ne != null) typeRight = ne.SemantMe();
		...
		if(typeLeft.isClass() && !typeRight.isInstanceOf(typeLeft))
		{
			...
			if (((TYPE_ARRAY) typeLeft).typeOfArray != typeRight) {
				throw new SEMANTIC_ERROR_EXCEPTION(String.valueOf(this.lineNumber));
			}
		}
		return null;
